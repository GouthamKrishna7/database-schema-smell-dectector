...
import { FileCode, Copy, Check, Download, Wand2, ShieldCheck, ArrowRight } from 'lucide-react';
...
export default function RefactorView({ migrationSql, refactoredSql, dialect }) {
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-lg flex flex-col">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between px-4 py-3 bg-slate-800/60 border-b border-slate-800 gap-3">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={handleCopy}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-medium border border-slate-700 transition active:scale-95"
          >
            {copied ? (
              <>
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-teal-600/20 hover:bg-teal-600/30 text-teal-300 text-xs font-medium border border-teal-500/30 transition active:scale-95"
          >
      ...
      {/* Info Banner */}
      <div className="px-4 py-2 bg-slate-950/80 border-b border-slate-800/80 text-xs text-slate-300 flex items-center gap-2">
        <span className="text-teal-400 font-semibold uppercase text-[11px]">
          {activeTab === 'migration' ? 'In-Place Migration' : 'Normalized Greenfield DDL'}:
        </span>
