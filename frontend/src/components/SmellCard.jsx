...
export default function SmellCard({ smell }) {
  ...
  const severityConfig = {
    CRITICAL: {
      bg: 'bg-rose-950/30 border-rose-900/50 hover:border-rose-700/60',
      badge: 'bg-rose-500/10 text-rose-300 border-rose-500/30',
      icon: <AlertOctagon className="w-4 h-4 text-rose-400 shrink-0" />,
      dot: 'bg-rose-500',
    },
    HIGH: {
      bg: 'bg-orange-950/25 border-orange-900/50 hover:border-orange-700/60',
  ...
  return (
    <div
      ...
      className={`rounded-xl border transition-all duration-200 cursor-pointer p-4 ${currentConfig.bg}`}
    >
      {/* Summary Row */}
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2 mb-1.5">
              ...
              <span className="text-xs font-mono font-medium text-teal-300 flex items-center gap-1 bg-teal-950/50 px-2 py-0.5 rounded border border-teal-900/50">
                <Table className="w-3 h-3 text-teal-400" />
                {smell.table_name}
                {smell.column_name && (
                  <span className="text-slate-400">({smell.column_name})</span>
                )}
              </span>
            </div>
      ...
      {isExpanded && (
        <div
          ...
          className="mt-4 pt-4 border-t border-slate-800/80 space-y-3 cursor-default"
        >
          {/* DBMS Theory */}
          ...
          {/* Impact */}
          <div className="bg-slate-950/70 p-3 rounded-lg border border-slate-800/80">
            <span className="text-[11px] font-bold uppercase tracking-wider text-rose-400 flex items-center gap-1.5 mb-1">
            ...
            <div className="bg-slate-950 p-3 rounded-lg border border-slate-800">
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-[11px] font-mono text-slate-400">
                  Recommended SQL Migration Fix:
                </span>
                <button
                  onClick={handleCopySQL}
                  className="flex items-center gap-1 text-[11px] px-2 py-0.5 rounded bg-slate-800 hover:bg-slate-700 text-slate-200 transition active:scale-95"
                >
                  {copied ? (
                    <>
              ...
              <pre className="font-mono text-xs text-teal-300 bg-slate-900/90 p-2.5 rounded border border-slate-800/80 overflow-x-auto selection:bg-teal-500/40">
                {smell.suggested_sql}
              </pre>
            </div>
          )}
